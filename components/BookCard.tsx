...
export function CaptureBookCard({ book }: { book: BookRecommendation }) {
  return (
    <div className="flex flex-col bg-white border border-[#1A1A1A]/10 rounded-2xl overflow-hidden h-[540px] w-full shadow-lg">
      <div className="relative h-[55%] w-full bg-[#F5F2ED] overflow-hidden">
        {book.thumbnail && (
        ...
        <div className="absolute top-4 right-4 bg-[#1A1A1A] text-white text-[10px] font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5">
          <Sparkle size={12} weight="fill" className="text-yellow-400" />
          {book.vibeScore}%
        </div>
      </div>
      <div className="p-6 flex flex-col flex-grow text-left">
        <span className="text-[10px] font-bold tracking-[0.25em] text-[#4A5D4E] uppercase mb-4">
          {book.genre || "General"}
        </span>
        <h3 className="font-serif text-lg font-bold text-[#1A1A1A] mb-1 leading-tight">
          {book.title}
        </h3>
        <p className="text-sm text-[#1A1A1A]/50 font-medium italic mb-4 text-left">
          {book.author}
        </p>
        <div className="pt-4 border-t border-[#1A1A1A]/5 mt-auto">
          <p className="text-[9px] font-serif italic text-[#4A5D4E]/60 line-clamp-3 text-left">
            "{book.reasoning}"
          </p>
        </div>
      </div>
